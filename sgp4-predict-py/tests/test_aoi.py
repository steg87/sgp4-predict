...
def test_areas_reject_non_finite_arguments(bad):
    """A latitude fails the range check, but nothing else has a range to fail.

    Unchecked, a `nan` is built into the shape and every offset is `nan`, which the
    scan floors to its minimum step — the whole interval ground through at a
    millisecond with no error raised. The library rejects it, and it has to reach
    Python as `ValueError` rather than a panic.
    """
    with pytest.raises(ValueError):
        Polygon([(0.0, 0.0), (10.0, bad), (10.0, 10.0)])
    with pytest.raises(ValueError):
        Polygon([(0.0, 0.0), (bad, 10.0), (10.0, 10.0)])
    with pytest.raises(ValueError):
        Rectangle((54.0, bad), (60.0, -1.0))
    with pytest.raises(ValueError):
        Rectangle((54.0, -8.0), (60.0, bad))
    with pytest.raises(ValueError):
...
def test_aoi_windows_compare_by_value():
    p = make_predictor()
    windows = list(p.aoi_iter(europe_polygon(), INTERVAL))
    again = list(p.aoi_iter(europe_polygon(), INTERVAL))
...
def test_non_finite_off_nadir_does_not_become_line_of_sight_reach():
    """NaN survives the clamp and would be read as the full horizon, so every
    line-of-sight pass would be reported as an access window."""
    p = make_predictor()
    area = Circle((52.0, 10.0), 1.0)
    nadir = list(p.aoi_iter(area, INTERVAL))
